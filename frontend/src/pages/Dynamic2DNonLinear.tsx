...
export default function Dynamic2DNonLinear() {
  ...
  const handleSolve = async () => {
    ...
    try {
      ...
      const payload = {
        eq_x: eqX,
        eq_y: eqY,
        mu: parseFloat(muParam),
        x0: parseFloat(x0),
        y0: parseFloat(y0),
        t_fin: parseFloat(tFin),
        h: parseFloat(hStep),
        x_min: -d,
        x_max: d,
        y_min: -d,
      ...
      setError(
        err.response?.data?.detail || "Error al resolver el sistema no lineal",
      );
    } finally {
      setLoading(false);
    }
  ...
  const phasePlotData = useMemo(() => {
    ...
    traces.push({
      ...
      line: { color: "#2563eb", width: 2, dash: "dash", smoothing: 1.3 }, // Smoothing extra
      showscale: false,
    ...
    traces.push({
      ...
      coloring: "none",
      contours: { coloring: "none", showlines: true, start: 0, end: 0 },
      line: { color: "#d97706", width: 2, dash: "dash", smoothing: 1.3 }, // Smoothing extra
      showscale: false,
  ...
  return (
    <div className="method-page">
      ...
      <div className="theory-section">
        ...
        <p>
          Determinación de estabilidad cualitativa mediante la linealización de
          la Matriz Jacobiana en vecindades de equilibrios aislados. Permite el
          análisis de bifurcaciones de 2D ajustando mu.
        </p>
      </div>
      ...
      <div className="method-container">
        <div className="form-section">
          ...
          >
            ...
            >
              Usa <strong>mu</strong> para introducir el parámetro de
              bifurcación.
            </small>
          </div>

          <h3 style={{ marginTop: "16px" }}>Parámetros de Estado</h3>
          <div
            style={{
              display: "grid",
            ...
            <>
              ...
              <div className="result-box">
                <div className="validation-title">
                  Sistemas Ecuaciones Original (Parseado SymPy)
                </div>
                <div
                  style={{
                    display: "flex",
                ...
                >
                  <FormulaDisplay
                    formula={`\\begin{cases} x' = ${resultado.ecuacion_latex_x} \\\\ y' = ${resultado.ecuacion_latex_y} \\end{cases}`}
                  />
                </div>
              </div>
              ...
              <div className="result-box">
                <div className="validation-title">
                  Matriz Jacobiana General Symbolic J(x,y)
                </div>
                <div style={{ padding: "10px 0" }}>
                  <FormulaDisplay
                    formula={`J(x,y) = ${resultado.jacobiano_latex}`}
                  />
                </div>
              </div>

              {/* Puntos de Equilibrio Nulclinas */}
              <div className="result-box">
                <div className="validation-title">
                  Puntos de Equilibrio Encontrados (f(x,y) = 0 ∩ g(x,y) = 0)
                </div>
                <div className="validation-box">
                  {resultado.puntos_analizados.length === 0 ? (
                    <div className="validation-row">
                      <span style={{ color: "#dc2626", fontWeight: "bold" }}>
                        No se interceptan las nulclinas en el plano real.
                      </span>
                    </div>
                  ) : (
                  ...
                  >
                    ...
                    <span>{resultado.nulclina_x_desp}</span>
                  </div>
                  <div className="validation-row">
                    <span className="validation-label">Nulclina dy/dt=0:</span>
                    <span>{resultado.nulclina_y_desp}</span>
                  </div>
                </div>
              </div>

              {/* Bloques de Análisis Cualitativo */}
              {resultado.puntos_analizados.map((pto, idx) => (
                <div
                  key={idx}
                ...
                >
                  ...
                  >
                    Análisis Cualitativo Local en P_{idx + 1}^*: (
                    {pto.x.toFixed(4)}, {pto.y.toFixed(4)})
                  </div>
                  <div className="validation-box">
                    <div
                      className="validation-row"
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <div>
                            {pto.jacobiano_local[0][0].toFixed(2)} &nbsp;&nbsp;{" "}
                            {pto.jacobiano_local[0][1].toFixed(2)}
                          </div>
                          <div>
                            {pto.jacobiano_local[1][0].toFixed(2)} &nbsp;&nbsp;{" "}
                            {pto.jacobiano_local[1][1].toFixed(2)}
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="validation-row">
                      ...
                      <span
                        style={{
                          fontWeight: "bold",
                          color:
                            pto.clasificacion.includes("stable") ||
                            pto.clasificacion.includes("Centro")
                              ? "#059669"
                              : "#dc2626",
                        }}
                      >
                    ...
                    <div className="validation-row">
                      ...
                      >
                        λ_1 = {pto.autovalores[0].real.toFixed(4)}{" "}
                        {pto.autovalores[0].imag >= 0
                          ? `+ ${pto.autovalores[0].imag.toFixed(4)}`
                          : `- ${Math.abs(pto.autovalores[0].imag).toFixed(4)}`}
                        i<br />
                        λ_2 = {pto.autovalores[1].real.toFixed(4)}{" "}
                        {pto.autovalores[1].imag >= 0
                          ? `+ ${pto.autovalores[1].imag.toFixed(4)}`
                          : `- ${Math.abs(pto.autovalores[1].imag).toFixed(4)}`}
                        i
                      </span>
                    </div>
                  </div>
